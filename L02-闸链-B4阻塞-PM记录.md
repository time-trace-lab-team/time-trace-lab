...
## 三之二、**旧 L02 任务卡已删除**（2026-09-14，项目方指令）

已从 develop 删除以下**旧几何**文档（3 板 2 门，已被「闸链」新图取代）：

```
L02-门房与双残影-PM裁决.md      （§三/§四/§十三 作废，整份移除）
L02-任务卡-开发1.md
L02-任务卡-开发2.md
L02-任务卡-开发3.md
L02-任务卡-PM接入.md
...
**仍有效的条款（原 L02 裁决 §十二）已搬迁至本节，凡引用「L02 裁决 §十二」的卡以本节为准**：

| 项 | 冻结内容 |
| --- | --- |
| `EchoTrail` 代际表达 | **不改签名** `EchoTrail(sourceRound, points, newer)`；轨迹语义 = 较新实线 `+2 px`、较旧虚线 `-2 px`，必须不依赖颜色可辨；`E1/E2` 文案留在 `ui/**` + `app/**`，不进 Canvas 数据 |
| `H` 增强 | **本批不做**，另行发卡 `RENDER-H-ECHO-EMPHASIS`（需先冻结 app 输入 + 只读显示状态形状）；README §三 关于 `H` 的描述保持不动 |
| 慢测口径 | `Level01MapRenderSnapshotTest` 允作**基线已知例外**：门禁 = 「与基线中位数相比涨幅 > 50% 才算回归」；测量时**不得与 JavaFX 实跑/构建并行**；报数须附 **≥3 次复跑与中位数**；**中位数连续 3 次 > 1 s** 才算回归 |
| 慢测归属 | 该用例在 `src/test/java/.../level/**` ⇒ **开发三**；开发一不得越界修改；收口见 `TECH-DEBT-SNAPSHOT-RUNTIME-v2-任务卡-开发3.md` |

保留未删（仍然在用）：`L02-B-射线投影接口-任务卡-开发1.md`（接口已实现并合入）、`TECH-DEBT-SNAPSHOT-RUNTIME-v2-任务卡-开发3.md`、本记录。
