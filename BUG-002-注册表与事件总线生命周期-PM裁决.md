...
### 8.2 §8.1 结案（PM，2026-09-13）

**裁决：保留实现，补进 README 并写清**（不调整实现）。理由：门槛只作用于**第一轮**，语义是「进入关卡后先读图、
第一次方向输入才开始读秒」，与 README §三 `READY` 冻结的意图一致，改动实现的收益不足以抵消回归风险。

- 已落地：`README.md` §三新增该条口径（「第一轮额外要求首个方向输入……后续轮次进入 `PLAYING` 即开始读秒」），
  经 **PR #73** 合入 `develop`（`1aa58ab`）；
- 已同步给测试：`EXPAND-MAP-VIEW-01-视口验收补充卡-测试.md` §四 第 5 条要求实测「第一轮不计时 + 第 2/3 轮立即读秒」；
